...
<div class="d-flex justify-content-between align-items-center mb-4">
  <div>
    <h4 class="fw-bold mb-1">Edit Carrier: {{ $carrier->name }}</h4>
    <p class="text-muted mb-0">Update carrier profile, equipment specifications, operating zones, and compliance documents</p>
  </div>
...
<form method="POST" action="{{ route('carriers.update', $carrier->id) }}" enctype="multipart/form-data">
  ...
  <div class="card shadow-sm mb-4">
    <div class="card-header border-bottom py-3 d-flex align-items-center">
      ...
      <h5 class="card-title mb-0">Carrier Profile</h5>
    </div>
        ...
        <div class="col-md-12">
          ...
          <select id="agent_id" class="form-select @error('agent_id') is-invalid @enderror" name="agent_id" required>
            ...
            @foreach($salesAgnets as $saleAgent)
              <option value="{{ $saleAgent->id }}" @selected(old('agent_id', $carrier->user_id) == $saleAgent->id)>
                {{ $saleAgent->first_name }} {{ $saleAgent->last_name }}
              </option>
            @endforeach
          </select>
          @error('agent_id')
      ...
      <div class="row g-3">
        <div class="col-md-4">
          ...
          <select id="truck_type" class="form-select @error('truck_type') is-invalid @enderror" name="truck_type" required>
            ...
            @foreach($truckTypes as $truckType)
              <option value="{{ $truckType->id }}" @selected(old('truck_type', $carrier->truck_type) == $truckType->id)>
                {{ $truckType->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="col-md-4">
          ...
          <select id="truck_size" class="form-select @error('truck_size') is-invalid @enderror" name="truck_size" required>
            ...
            @foreach($truckSizes as $truckSize)
              <option value="{{ $truckSize->id }}" @selected(old('truck_size', $carrier->truck_size) == $truckSize->id)>
                {{ $truckSize->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="col-md-4">
          ...
          <div class="input-group">
            <input
              ...
              name="maximum_weight"
              value="{{ old('maximum_weight', $carrier->maximum_weight) }}"
              required
            />
        ...
        <div class="col-md-3">
          ...
          <select id="payment_type" class="form-select @error('payment_type') is-invalid @enderror" name="payment_type" required>
            ...
            @foreach($paymentTypes as $paymentType)
              <option value="{{ $paymentType->id }}" @selected(old('payment_type', $carrier->payment_type) == $paymentType->id)>
                {{ $paymentType->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="col-md-3">
          ...
          <select id="percent_flat" class="form-select @error('percent_flat') is-invalid @enderror" name="percent_flat" required>
            ...
            <option value="percentage" @selected(old('percent_flat', $carrier->percent_flat) === 'percentage')>Percentage (%)</option>
            <option value="flat_rate" @selected(old('percent_flat', $carrier->percent_flat) === 'flat_rate')>Flat Rate ($)</option>
          </select>
          @error('percent_flat')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        ...
        <div class="col-md-3">
          ...
          <input
            ...
            name="charge_type"
            value="{{ old('charge_type', $carrier->charge_type) }}"
            required
          />
          @error('charge_type')
            <div class="invalid-feedback">{{ $message }}</div>
    ...
    <div class="card-body pt-4">
      <div class="mb-3">
        <div class="form-check form-switch">
          <input class="form-check-input" type="checkbox" id="all_zones" name="all_zones" @checked(old('all_zones', $carrier->all_zones))>
          <label class="form-check-label fw-bold" for="all_zones">Select All Zones (Nationwide)</label>
        </div>
      </div>
      <div class="row g-3">
        @for($i = 0; $i <= 9; $i++)
          @php $zField = 'z' . $i; @endphp
          <div class="col-6 col-sm-4 col-md-2">
            <div class="form-check form-switch">
              <input
                ...
                id="z{{ $i }}"
                name="z{{ $i }}"
                @checked(old($zField, $carrier->$zField))
              >
      ...
      <div class="row g-3">
        <div class="col-md-8">
          ...
          <input
            ...
            name="street_address"
            value="{{ old('street_address', $carrier->street_address) }}"
            required
          />
        ...
        <div class="col-md-4">
          ...
          <input
            ...
            name="city_name"
            value="{{ old('city_name', $carrier->city_name) }}"
            required
          />
          @error('city_name')
            <div class="invalid-feedback">{{ $message }}</div>
        ...
        <div class="col-md-6">
          ...
          <select class="form-select @error('state_name') is-invalid @enderror" name="state_name" id="state_name" required>
            ...
            @foreach($states as $state)
              <option value="{{ $state->id }}" @selected(old('state_name', $carrier->state_name) == $state->id)>
                {{ $state->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="col-md-6">
          ...
          <input
            ...
            name="zip_code"
            value="{{ old('zip_code', $carrier->zip_code) }}"
            required
          />
          @error('zip_code')
            <div class="invalid-feedback">{{ $message }}</div>
  ...
  <div class="card shadow-sm mb-4">
    ...
    <div class="card-body pt-4">
      <div class="row g-3">
        @php
          $docs = [
            ['name' => 'mc_letter', 'label' => 'MC Authority Letter', 'val' => $carrier->mc_letter],
            ['name' => 'w_form', 'label' => 'W-9 Form (Tax ID Proof)', 'val' => $carrier->w_form],
            ['name' => 'coi', 'label' => 'COI (Certificate of Insurance)', 'val' => $carrier->coi],
            ['name' => 'noa', 'label' => 'NOA (Notice of Assignment)', 'val' => $carrier->noa],
            ['name' => 'void_cheque', 'label' => 'VOID Cheque', 'val' => $carrier->void_cheque],
            ['name' => 'extra_document', 'label' => 'Extra Document', 'val' => $carrier->extra_document],
          ];
        @endphp
        ...
        @foreach($docs as $doc)
          <div class="col-md-4">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <label for="{{ $doc['name'] }}" class="form-label mb-0">{{ $doc['label'] }}</label>
              @if($doc['val'])
                @php $ext = pathinfo($doc['val'], PATHINFO_EXTENSION); @endphp
                <a
                  href="{{ asset('carrier_img/' . $doc['val']) }}"
                  target="_blank"
            ...
            <input id="{{ $doc['name'] }}" type="file" class="form-control @error($doc['name']) is-invalid @enderror" name="{{ $doc['name'] }}">
            @error($doc['name'])
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <small class="text-muted">Choose a new file to replace current</small>
          </div>
        @endforeach
      </div>
    </div>
      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <textarea
            ...
            name="comment"
          >{{ old('comment', $carrier->comment) }}</textarea>
          @error('comment')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
  ...
  <div class="d-flex justify-content-end gap-2 mb-4">
    ...
    <button type="submit" class="btn btn-primary">
      <i class="ti ti-device-floppy me-1"></i> Update Carrier
    </button>
  </div>
</form>
