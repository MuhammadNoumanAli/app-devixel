...
<div class="card shadow-sm">
  <div class="card-header border-bottom py-3 d-flex justify-content-between align-items-center">
    ...
    <div style="width: 250px;">
        <select id="select_mc_numbers_not_paid" class="form-select form-select-sm" onchange="filterUnpaidByMC(this.value)">
          <option value="">-- All MC Numbers --</option>
          @foreach($mc_numbers as $row)
            <option value="{{ $row->mc_number }}" {{ request('mc_number') == $row->mc_number ? 'selected' : '' }}>{{ $row->mc_number }} - {{ $row->company_name }}</option>
          @endforeach
        </select>
      </div>
    </div>
    ...
    <div id="unpaid_pagination">
      @include('layouts.pagination', ['paginator' => $dispatchers, 'name' => 'unpaid invoices'])
    </div>
  </div>

  @push('scripts')
  <script>
    function filterUnpaidByMC(mc) {
      var url = new URL(window.location.href);
      if (mc) {
        url.searchParams.set('mc_number', mc);
      } else {
        url.searchParams.delete('mc_number');
      }
